...
var config = {
    ...
    storageBucket: "family-to-do-list-5ca3b.appspot.com",
    messagingSenderId: "261964055081"
  };
...
var ftdl = {
	...
	saveNote: function (e) {
		...
		var todoNumber = $(this).attr("todoID");
		if (note.length > 0) {
			database.ref('/Users/' + firebase.auth().currentUser.uid + '/list/' + todoNumber + '/note').push({
				"note": note,
				"name": currentMember
			});
		}
	...
	appendList: function (todoInfo, id, timed) {
		...
		var $img1 = $('<img>').attr({
				'src': 'assets/images/timed_event.jpg',
				'title': 'Timed Events'
			})
			.addClass('link-icon');
		var $img2 = $('<img>').attr({
				...
				'title': 'Map',
				'data-address': todoInfo.LocationInfo.address
			})
			.addClass('link-icon rtrvadd').bind('click', ftdl.retrieveAddress);
		var $img3 = $('<img>').attr({
	...
	appendEvent: function (eventInfo, id) {
		...
		var $img1 = $('<img>').attr({
				...
				'data-target': '#mapmodal',
				'title': 'Location',
				'data-address': eventInfo.LocationInfo.address
			})
